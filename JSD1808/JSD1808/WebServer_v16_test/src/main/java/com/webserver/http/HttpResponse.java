...
public class HttpResponse {
	...
	private void sendStatusLine() {
		//1、发送状态行
		String line = "HTTP/1.1 "+statusCode+statusReason;//发送的内容
		println(line);
	...
	private void sendContent() {
		//3、发送响应正文内容
		/*
		 *添加一个新的分支，若contentDate这个属性不为null时，
		 *则将该字节数组内容作为响应正文内容发送给客户端。
		 */
		  if(contentData != null) {
			try {
			...
			}else if(entity != null) {//注册业务并不能打开文件，只有不是注册业务时才打开
			try(
	...
	/**
	 *设置状态代码，设置的同时会自动设置该状态代码的默认对应的状态描述。
	 * @param statusCode
	 */
